...
const StudentDashboard: React.FC = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string | 'ALL' | 'SAVED'>(
    searchParams.get('view') === 'vault' ? 'SAVED' : 'ALL'
  );
  ...
  // Filter categories based on student permissions
  const permittedCategories = useMemo(() => {
    if (!user?.permissions) return [];
    if (user.permissions.allAccess) return categories;
    return categories.filter(cat => 
      user.permissions?.categories.includes(cat.id) && 
  ...
  const accessibleDocuments = documents.filter(doc => {
    ...
    const isPermitted = user?.permissions?.allAccess || docCategoryIds.some(cid => 
    ...
    return isPublished && isPermitted && matchesCategory && matchesSearch;
  });
  ...
  const statItems = [
    { label: 'NODES', val: accessibleDocuments.length, color: 'text-white', icon: <Database size={16} className="shrink-0" />, desc: 'STORAGE' },
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 border-b border-white/5 pb-6 sm:pb-8">
        <div className="space-y-2 sm:space-y-3">
          <div className="flex items-center gap-3 text-accent">
            ...
            <p className="text-[8px] sm:text-[10px] font-heading tracking-[0.5em] uppercase font-black">ARCHIVE</p>
          </div>
          <h2 className="text-2xl sm:text-5xl font-heading font-black tracking-tighter text-white uppercase leading-none">LIBRARY</h2>
        </div>
              ...
              <div key={doc.id} className="group relative bg-black/40 border-b border-white/5 hover:bg-white/5 transition-all">
                 <div className="flex items-center gap-4 p-3 h-16 sm:h-20">
                    ...
                    <div className="flex-1 min-w-0 flex items-center gap-3">
                        ...
                        <span className="shrink-0 text-[9px] text-muted/40 font-heading uppercase tracking-wider border border-white/5 px-2 py-1 rounded hidden sm:block">{getCategoryName(doc)}</span>
                    </div>
            ...
            <div key={doc.id} className="relative group bg-black border border-white/10 p-px transition-all duration-500 hover:border-accent hover:-translate-y-1" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 85%, 85% 100%, 0 100%)' }}>
              <div className="bg-surface/40 flex flex-col h-full relative group-hover:bg-surface/60">
                ...
                <div className="relative aspect-video overflow-hidden border-b border-white/5 bg-zinc-950">
                   <img src={doc.thumbnail || undefined} className="w-full h-full object-cover grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-1000 group-hover:scale-110" alt="Res" referrerPolicy="no-referrer" />
                ...
                <div className="p-8 space-y-6 flex-1 flex flex-col">
                  ...
                  <p className="text-[9px] font-heading text-muted uppercase tracking-[0.2em] font-black opacity-40 mt-auto">{getCategoryName(doc)}</p>
                </div>
                <Link to={generateDocumentUrl(doc)} className="w-full py-5 bg-white/5 border-t border-white/5 text-[10px] font-heading font-black text-white/60 uppercase tracking-[0.4em] hover:bg-accent hover:text-black transition-all flex items-center justify-center gap-2">VIEW <Maximize2 size={14} /></Link>
              </div>
            </div>
          );
        })}
